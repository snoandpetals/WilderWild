...
public final class RegisterSounds {
    ...
    public static SoundEvent register(String path) {
        return Registry.register(Registry.SOUND_EVENT, WilderWildFabric.id(path), new SoundEvent(WilderWildFabric.id(path)));
    }

    public static SoundEvent registerStarting(String path) {
        var sound = register(path);
        Registry.register(FrozenRegistry.STARTING_SOUND, WilderWildFabric.id(path), sound);
        return sound;
    }

    public static void init() {
        WilderWildFabric.logWild("Registering SoundEvents for", WilderWild.UNSTABLE_LOGGING);
    }
