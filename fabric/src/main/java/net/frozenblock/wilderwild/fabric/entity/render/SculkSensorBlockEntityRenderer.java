...
import net.frozenblock.wilderwild.fabric.misc.SculkSensorTickInterface;
...
public class SculkSensorBlockEntityRenderer<T extends SculkSensorBlockEntity> implements BlockEntityRenderer<T> {
    ...
    private static final RenderType SENSOR_LAYER = RenderType.entityCutout(WilderWildFabric.id("textures/entity/sculk_sensor/inactive.png"));
    private static final RenderType ACTIVE_SENSOR_LAYER = RenderType.entityCutout(WilderWildFabric.id("textures/entity/sculk_sensor/active.png"));

    public SculkSensorBlockEntityRenderer(Context ctx) {
        ModelPart root = ctx.bakeLayer(WilderWildClientFabric.SCULK_SENSOR);
