...
import net.frozenblock.wilderwild.fabric.registry.RegisterParticles;
...
public class PollenBlock extends FlowerLichenBlock {
    ...
    public static boolean canAttachTo(BlockGetter level, Direction direction, BlockPos pos, BlockState state) {
        return isFaceFull(state.getBlockSupportShape(level, pos), direction.getOpposite()) || isFaceFull(state.getCollisionShape(level, pos), direction.getOpposite()) && !level.getBlockState(pos).is(Blocks.WATER);
    }
}
