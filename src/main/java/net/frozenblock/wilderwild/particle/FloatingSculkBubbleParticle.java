...
public class FloatingSculkBubbleParticle extends RisingParticle {
    ...
    protected FloatingSculkBubbleParticle(ClientLevel clientLevel, double x, double y, double z, double size, double maxAge, double yVel, SpriteSet spriteProvider) {
        ...
        this.xd = (MathUtil.random().nextDouble() - 0.5) / 9.5;
        this.zd = (MathUtil.random().nextDouble() - 0.5) / 9.5;
        this.spriteProvider = spriteProvider;
        this.setSpriteFromAge(spriteProvider);
        this.yd = yVel;
        this.sound = size <= 0 ? WWSoundEvents.PARTICLE_FLOATING_SCULK_BUBBLE_POP.get() : WWSoundEvents.PARTICLE_FLOATING_SCULK_BUBBLE_BIG_POP.get();
        if (size >= 1) {
            this.scale((float) (1.4F + size));
            this.xd = (MathUtil.random().nextDouble() - 0.5) / 10.5;
            this.zd = (MathUtil.random().nextDouble() - 0.5) / 10.5;
        }
