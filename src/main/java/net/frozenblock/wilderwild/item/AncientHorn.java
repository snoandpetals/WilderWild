    ...
    public InteractionResultHolder<ItemStack> use(@NotNull Level level, @NotNull Player user, @NotNull InteractionHand hand) {
        ...
        if (optional.isPresent()) {
            ...
            if (level instanceof ServerLevel server) {
                ...
                Objects.requireNonNull(FrozenPackets.movingRestrictionSoundS2C(server, projectileEntity, RegisterSounds.ENTITY_ANCIENT_HORN_PROJECTILE_LOOP, SoundSource.NEUTRAL, 1.0F, 1.0F, FrozenMain.id("default"), true)).send(server);
