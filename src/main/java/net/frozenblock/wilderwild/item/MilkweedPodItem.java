...
public class MilkweedPodItem extends Item {
    ...
    public MilkweedPodItem(Properties settings) {
        super(settings);
    }
    ...
    public InteractionResultHolder<ItemStack> use(@NotNull Level level, @NotNull Player user, @NotNull InteractionHand hand) {
        ...
        if (level instanceof ServerLevel server) {
            ...
            NetworkUtil.sendControlledParticle(level, user.getEyePosition().add(0, -0.1, 0), f, g, h, server.random.nextIntBetweenInclusive(5, 20), true, 48);
        }
