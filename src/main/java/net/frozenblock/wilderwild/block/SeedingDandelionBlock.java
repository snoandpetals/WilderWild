...
public class SeedingDandelionBlock extends FlowerBlock {
    ...
    public void animateTick(@NotNull BlockState state, @NotNull Level level, @NotNull BlockPos pos, RandomSource random) {
        if (random.nextFloat() > 0.95) {
            level.addParticle(WWParticles.DANDELION_SEED.get(), pos.getX() + 0.5, pos.getY() + 0.8, pos.getZ() + 0.5, 0.0D, 0.0D, 0.0D);
        }
    }
    ...
    public void entityInside(@NotNull BlockState state, @NotNull Level level, @NotNull BlockPos pos, @NotNull Entity entity) {
        if (level instanceof ServerLevel server) {
            if (server.random.nextFloat() > 0.95) {
                NetworkUtil.sendSeedParticlePacket(level, Vec3.atCenterOf(pos).add(0, 0.3D, 0), server.random.nextIntBetweenInclusive(1, 3), false, 32);
            }
        }
    }
    ...
    public void playerWillDestroy(@NotNull Level level, @NotNull BlockPos pos, @NotNull BlockState state, @NotNull Player player) {
        ...
        if (level instanceof ServerLevel server) {
            NetworkUtil.sendSeedParticlePacket(level, Vec3.atCenterOf(pos).add(0, 0.3D, 0), server.random.nextIntBetweenInclusive(3, 7), false, 32);
        }
    }
}
