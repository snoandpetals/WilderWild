...
public class AlgaeFeature extends Feature<ProbabilityFeatureConfiguration> {
    ...
    public boolean place(FeaturePlaceContext<ProbabilityFeatureConfiguration> context) {
        ...
        BlockPos s = blockPos.atY(level.getHeight(Heightmap.Types.MOTION_BLOCKING_NO_LEAVES, blockPos.getX(), blockPos.getZ()));
        int y = s.getY();
                        ...
                        if (random.nextFloat() > 0.2F) {
                            ...
                            if (fade) {
                                if (random.nextFloat() > 0.5F) {
                                    level.setBlock(mutableDisk, WWBlocks.ALGAE.get().defaultBlockState(), 3);
                                    bl = true;
                                }
                            } else {
                                level.setBlock(mutableDisk, WWBlocks.ALGAE.get().defaultBlockState(), 3);
