package net.frozenblock.wilderwild.world.feature.decorators;
...
public class ShelfFungusTreeDecorator  extends TreeDecorator {
    ...
    protected TreeDecoratorType<?> type() {
        return WWTreeDecoratorTypes.FUNGUS_TREE_DECORATOR.get();
    }
                    ...
                    if (abstractRandom.nextFloat() <= 0.25F) {
                        ...
                        if (generator.isAir(blockPos)) {
                            if (generator.random().nextFloat() < redChance) {
                                generator.setBlock(blockPos, WWBlocks.RED_SHELF_FUNGUS.get().defaultBlockState().setValue(ShelfFungusBlock.STAGE, abstractRandom.nextInt(3) + 1).setValue(ShelfFungusBlock.FACE, AttachFace.WALL).setValue(ShelfFungusBlock.FACING, direction));
                            } else {
                                generator.setBlock(blockPos, WWBlocks.BROWN_SHELF_FUNGUS.get().defaultBlockState().setValue(ShelfFungusBlock.STAGE, abstractRandom.nextInt(3) + 1).setValue(ShelfFungusBlock.FACE, AttachFace.WALL).setValue(ShelfFungusBlock.FACING, direction));
                            }
                        }
                    }
                }
