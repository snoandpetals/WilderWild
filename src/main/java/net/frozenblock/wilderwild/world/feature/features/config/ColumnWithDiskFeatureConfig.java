...
public class ColumnWithDiskFeatureConfig implements FeatureConfiguration {
    public static final Codec<ColumnWithDiskFeatureConfig> CODEC = RecordCodecBuilder.create((instance) -> {
        ...
        }), IntProvider.NON_NEGATIVE_CODEC.fieldOf("height2").forGetter((config) -> {
            return config.height2;
        }), RegistryCodecs.homogeneousList(Registry.BLOCK_REGISTRY).fieldOf("replaceable").forGetter((config) -> {
            return config.replaceable;
        }), RegistryCodecs.homogeneousList(Registry.BLOCK_REGISTRY).fieldOf("diskBlocks").forGetter((config) -> {
